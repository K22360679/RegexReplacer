...
def main():
    ...
    def AdvancedOption():
        ...
        ReplaceFileCheckBox = tk.Checkbutton(
            AdvancedOptionFrame, variable=ReplaceFileFlag)
        ...
        CancelButton = tk.Button(
            ButtonsFrame, text="キャンセル", command=Cancel_CMD)
    ...
    ClearButton = tk.Button(FileFrame, text="×",
                            command=lambda: FileEntry.delete(0, tk.END))
    ClearButton.grid(row=0, column=2)

    def inputFilePass():
        FilePass.set(filedialog.askopenfilename(
    ...
    isExistsEntry = tk.Entry(
        FileFrame, textvariable=isExistsStr, state="readonly", width=80)
    isExistsEntry.grid(row=1, column=1)
    FileDialogButton = tk.Button(
        FileFrame, command=inputFilePass, text="ファイルを選択")
    ...
    RegexErrorChecker = tk.Entry(
        RegexFrame, state="readonly", textvariable=Checker, width=25)
    ...
    ReplacedEntry = tk.Entry(
        ReplaceFrame, textvariable=Replaced, state="readonly", width=25)
    ...
    RunningButton = tk.Button(RunningFrame, text="Run",
                              command=RunningCMD, height=2, width=40)
